<template>
  <div class="height-100 cityManage">
    <M-Table-List >
      <div slot="Info">
        <el-form :inline="true" :model="searchForm" size="mini">
          <el-form-item label="城市名称：">
            <el-input v-model.trim="searchForm.name" :style="{width:formInputWidth}" placeholder="城市名称" clearable></el-input>
          </el-form-item>
          <el-form-item>
      ...
      <div slot="TableDom">
        <el-table :data="tableData" :stripe="true"  style="width: 100%;"
          ...
          <el-table-column prop="name" label="城市名称" min-width="100" align="center">
          </el-table-column>
          <el-table-column prop="code" label="城市编码" min-width="100" align="center">
          </el-table-column>
    ...
    <!--新增弹窗-->
    <div>
      <el-dialog :title="'城市管理-新增'" custom-class="dialog-type-column2" :visible.sync="addDialog" :close-on-click-modal="false" @close="closed">
        <div :v-loading="loading">
          <el-form :model="ruleForm" ref="ruleForm" label-position="right" label-width="110px" :rules="rules">
            <el-form-item label="省份名称：" prop="parentId" :label-width="formLabelWidth">
              <el-select v-model="ruleForm.parentId" placeholder="请选择省份"  style="width: 100%;">
                <el-option v-for="item in provinceList" :key="item.id" :label="item.name" :value="item.id"></el-option>
              </el-select>
            </el-form-item>
            <el-form-item label="城市名称：" prop="name" :label-width="formLabelWidth">
              <el-input placeholder="城市名称" v-model.trim="ruleForm.name" style="width: 100%;" maxlength="32"></el-input>
            </el-form-item>
            <el-form-item label="城市编码：" prop="code" :label-width="formLabelWidth">
              <el-input placeholder="城市编码" v-model.trim="ruleForm.code" style="width: 100%;" maxlength="32"></el-input>
            </el-form-item>
    ...
    <!-- 编辑弹窗 -->
    <div>
      <el-dialog :title="'城市管理-编辑'" custom-class="dialog-type-column2" :visible.sync="editDialog" :close-on-click-modal="false" @close="closed">
        <div :v-loading="loading">
          <el-form :model="ruleForm" ref="ruleForm" label-position="right" label-width="110px" :rules="rules">
            <el-form-item label="省份名称：" prop="parentId" :label-width="formLabelWidth">
              <el-select v-model="ruleForm.parentId" placeholder="请选择省份"  style="width: 100%;">
  ...
  export default {
    ...
    data() {
      return {
        ...
        // 必填项校验
        rules: {
          parentId: [{required: true,message: '请选择省份',trigger: 'change'}],
          name: [{required: true,message: '请输入城市名称',trigger: ['blur','change']}],
    ...
    methods: {
      ...
      // 获取省份列表
      getProvinceList(){
        this.$axios.post("baseProvince/findBaseProvince", this.searchForm).then(res=> {
          if(res.data.code==200) {
            this.provinceList = res.data.data.rows;
          }
        })
      },
      // 重置信息
      ...
      refreshData: function() {
        ...
        this.$axios.post("baseCity/findBaseCity", this.searchForm).then(function(res) {
          if(res.data.code==200) {
        ...
        setTimeout(() => {
          ...
          this.$refs.ruleForm.validate(valid=>{
            if(valid){
              ...
              let that = this;
                that.loading=true
                this.$axios.post("baseCity/addBaseCity", params).then(function(res) {
                  that.loading=false
                  if(res.data.code==200) {
      ...
      editClick(){
        ...
        setTimeout(() => {
          ...
          this.$refs.ruleForm.validate(valid=>{
            ...
            let that = this;
            that.loading=true
            this.$axios.post("baseCity/updateBaseCity", params).then(function(res) {
                that.loading=false
                if(res.data.code==200) {
                  that.$message({
                    message: '编辑成功',
      ...
      this.$confirm("即将删除选中的数据, 是否继续?", "提示", {
        ...
        type: "warning"
          ...
          this.$axios
            .post("baseCity/deleteBaseCity", { id: row.id })
            .then(res => {
              if (res.data.code==200) {
                this.$message({
                  message: '删除成功',
