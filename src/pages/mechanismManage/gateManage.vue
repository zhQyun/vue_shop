<template>
    <div class="gateManage">
      ...
      <div slot="TableDom" v-loading="loading">
        ...
        >
          <el-table-column prop="number" label="序号" min-width="100" align="center">
             <template slot-scope="scope">
                {{scope.$index+1}}
            </template>
          </el-table-column>
          <el-table-column prop="code" label="机构编号" min-width="100" align="center">
          </el-table-column>
          <el-table-column prop="organName" label="所属机构" min-width="120" align="center">
          </el-table-column>
          <el-table-column prop="count" label="闸机数" min-width="120" align="center">
          </el-table-column>
          <el-table-column label="操作"  width="450">
            <template slot-scope="scope">
                <el-link type="primary" class="link" @click="details(scope.row)" >闸机详情</el-link>
            </template>
          </el-table-column>
        </el-table>
      </div>
 ...
 */
    ...
    methods:{
        // 获取列表信息
        getData(){
          this.$axios.post('baseGateManage/findByOrganBaseGateManage',this.form).then(res=>{
        ...
        //查看详情
        details(row){
             this.$router.push({name:'searchGateManage',query: {id:row.id,title:row.organName}})
        },
    }
}
</script>
