<template>
  <div class="height-100">
    <M-Table-List >
      <div slot="Info">
        <el-form :inline="true" :model="searchForm" size="mini">
          ...
          <el-form-item label="设备编号(对内)：">
            <el-input v-model.trim="searchForm.deviceNo" :style="{width:formInputWidth}" placeholder="设备编号(对内)" clearable></el-input>
          </el-form-item>
          <el-form-item>
      ...
      <div slot="TableDom">
        <el-table :data="tableData" :stripe="true"  style="width: 100%;"
          ...
          <el-table-column prop="deviceNo" label="设备编号(对内)" min-width="150" align="center">
          </el-table-column>
          <el-table-column prop="chanId" label="摄像头通道" min-width="150" align="center">
          </el-table-column>
    ...
    <!--新增弹窗-->
    <div>
      <el-dialog :title="'摄像头关系-新增'" custom-class="dialog-type-column2" :visible.sync="addDialog" :close-on-click-modal="false" @close="closed">
        <div :v-loading="loading">
          <el-form :model="ruleForm" ref="ruleForm" label-position="right" label-width="110px" :rules="rules">
            ...
            <el-form-item label="设备编号(对内)：" prop="deviceNo" :label-width="formLabelWidth" >
              <el-input v-model.trim="ruleForm.deviceNo" placeholder="设备编号(对内)" style="width: 100%;" maxlength="32"></el-input>
            </el-form-item>
            <el-form-item label="摄像头通道：" prop="chanId" :label-width="formLabelWidth">
              <el-input v-model.trim="ruleForm.chanId" placeholder="摄像头通道" style="width: 100%;" maxlength="32">
              </el-input>
            </el-form-item>
          </el-form>
        </div>
    ...
    <!-- 编辑弹窗 -->
    <div>
      <el-dialog :title="'摄像头关系-编辑'" custom-class="dialog-type-column2" :visible.sync="editDialog" :close-on-click-modal="false" @close="closed">
        <div :v-loading="loading">
          <el-form :model="ruleForm" ref="ruleForm" label-position="right" label-width="110px" :rules="rules">
              <el-form-item label="学校名称：" prop="schoolId" :label-width="formLabelWidth" >
  ...
  export default {
    ...
    data() {
      return {
        ...
        rules: {
          ...
          deviceNo: [{required: true, message: '请输入设备编号(对内)', trigger: ['blur','change']}],
          chanId: [{required: true, message: '请输入摄像头通道', trigger: ['blur','change']}],
        },
      }
    },
    created: function() {
    ...
    methods: {
      ...
      refreshData: function() {
        ...
        this.$axios.post("/baseCameraSchool/findBaseCameraSchool", this.searchForm).then(function(res) {
        ...
        setTimeout(() => {
          ...
          this.$refs.ruleForm.validate(valid=>{
            if(valid){
              ...
              let that = this;
                that.loading=true
                this.$axios.post("/baseCameraSchool/addBaseCameraSchool", params).then(function(res) {
                  that.loading=false
                  if(res.data.code==200) {
                    that.$message({
                      message: '新增成功',
      ...
      editClick(){
        ...
        setTimeout(() => {
          ...
          this.$refs.ruleForm.validate(valid=>{
            ...
            let that = this;
            that.loading=true
            this.$axios.post("/baseCameraSchool/updateBaseCameraSchool", params).then(function(res) {
                that.loading=false
                if(res.data.code==200) {
                  that.$message({
                    message: '编辑成功',
      ...
      this.$confirm("即将删除选中的数据, 是否继续?", "提示", {
        ...
        type: "warning"
          ...
          this.$axios
            .post("/baseCameraSchool/deleteBaseCameraSchool", { id: row.id ,schoolId:row.schoolId})
            .then(res => {
              if (res.data.code==200) {
                this.$message({
                  message: '删除成功',
